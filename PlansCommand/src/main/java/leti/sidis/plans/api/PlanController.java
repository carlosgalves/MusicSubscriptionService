...
import leti.sidis.plans.amqp.AmqpReceiver;
...
import leti.sidis.plans.model.Role;
...
public class PlanController {
    ...
    @RolesAllowed(Role.MARKETING_DIRECTOR)
    public ResponseEntity<PlanDTO> createPlanBonus(@PathVariable("userId")
            final UUID userId,
            @Valid @RequestBody final CreatePlanRequest resource) {


        final var plan = planService.createPlanBonus(resource, userId);
        return ResponseEntity.ok().eTag(Long.toString(plan.getVersion())).body(planMapper.toPlanDTO(plan));
    }
